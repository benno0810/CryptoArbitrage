...
class EosBtcWithoutLoop(Performance):

    def __init__(self, Market):
        os.environ[Constants.DQUANT_ENV] = "dev"
        self.mkt = Market("eos_btc")
    ...
    def buy(self):
        res_b = self.mkt.buy(10, price=0.00001)
        return res_b


class EosBtcWithLoop(Performance):

    def __init__(self, Market):
        os.environ[Constants.DQUANT_ENV] = "dev"
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
        self.mkt = Market("eos_btc", loop)
        self.mkt.setDaemon(True)
        self.mkt.start()
        time.sleep(10)
        super().__init__(self.mkt)
...
class OkexEosBtcWithLoop(Performance):
    ...
    def buy(self):
        res_b = self.mkt.buy(0.00001, amount=10)
        return res_b


def test_bi():
    bi = EosBtcWithoutLoop(Binance)
    bi.run()

def test_huobi_bfx_okex():
    hb_ws = EosBtcWithLoop(HuobiWs)
    hb_ws.setDaemon(True)

    bfx = EosBtcWithLoop(BitfinexSpotWs)
    bfx.setDaemon(True)

    okex = OkexEosBtcWithLoop(OkexSpotWs)
    okex.setDaemon(True)
