...
class OkexSpotWs(Market):
    ...
    def buy(self, price, amount=-1):
        ...
        if self.trade['order_id'] in self.trade_data_chache:
            self.sync_update_sub_trades(self.trade_data_chache[self.trade['order_id']])
        ...
        while True:
            try:
                ...
                try:
                    ...
                    continue
                # if self.delete_order_result and 'result' in self.delete_order_result:
                #     if self.delete_order_result['result'] is True:
                #         self._remove_from_active_order(order_id)
                #         ret = self.getOrder(order_id)
                #         logger.debug("OKEXSpot deleteOrder: %s" % ret)
                #         return ret
                order_details = self.getOrder(order_id)
            ...
            try:
                if order_id:
                    ...
                    try:
                        ...
                        self.error('OKEX getOrder timeout, retry')
                        continue
                    order = copy(self.order)
                    self.order = None
                    # logger.debug("OKEX getOrder: %s" % order)
...
if __name__ == '__main__':
    ...
    ok = OkexSpotWs('eth_btc', loop)
    ok.setDaemon(True)
    ok.start()
    print(ok.sell(amount=0.01))
