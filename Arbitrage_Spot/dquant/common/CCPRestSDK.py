...
import json
from dquant.common.xmltojson import xmltojson
import requests
...
class REST:
    ...
    def sendTemplateSMS(self, to,datas,tempId):
        ...
        src = self.AccountSid + ":" + self.Batch
        #auth = base64.encodestring(src).strip()
        auth = base64.encodebytes(src.encode('utf8')).strip()
        #req = urllib.request.Request(url)
        #self.setHttpHeader(req)
        #req.add_header("Authorization", auth)
        #创建包体
        ...
        data=''
        try:
            params = {"to": to, "templateId": str(tempId), "appId": self.AppId}
            if datas:
                params['datas'] = json.dumps(datas)
            headers = {'Content-Type': 'application/json', 'Authorization': auth, 'Accept': 'application/json', 'Content-Length':  '1000'}
            res = requests.post(url, headers=headers, data=json.dumps(params))
            data = res.text
            if self.BodyType=='json': #json格式
                locations = res.json()
            else: # xml格式
                xtj=xmltojson()
                locations=xtj.main(data)
